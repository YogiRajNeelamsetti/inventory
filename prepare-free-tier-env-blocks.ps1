...
$dbMaxPoolSize = Get-EnvOrPlaceholder -Map $envMap -Key "DB_MAX_POOL_SIZE" -Placeholder "2" -HideSecret:$false
$dbMinIdle = Get-EnvOrPlaceholder -Map $envMap -Key "DB_MIN_IDLE" -Placeholder "0" -HideSecret:$false
$dbConnectionTimeoutMs = Get-EnvOrPlaceholder -Map $envMap -Key "DB_CONNECTION_TIMEOUT_MS" -Placeholder "10000" -HideSecret:$false
$flywayConnectRetries = Get-EnvOrPlaceholder -Map $envMap -Key "FLYWAY_CONNECT_RETRIES" -Placeholder "20" -HideSecret:$false
...
Write-Output "DB_CONNECTION_TIMEOUT_MS=$dbConnectionTimeoutMs"
